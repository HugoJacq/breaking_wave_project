/*
 ...
 Original code : Jiarong Wu, https://github.com/jiarong-wu/multilayer_breaking/blob/main/specgen/specgen.py
...
T_Spectrum spectrum_gen_linear(int N_mode, int N_power, double L, double P, double kp) {
  ...
  */
  ...
  spectrum.kmod = malloc(N_kmod * sizeof(double));
  spectrum.kx = malloc(N_mode * sizeof(double));
  spectrum.ky = malloc((N_mode+1) * sizeof(double));
  spectrum.F_kmod = malloc(N_kmod * sizeof(double));
  spectrum.F_kxky = malloc(N_mode * (N_mode+1) * sizeof(double));

  
  for (int i=0; i< N_kmod; ++i){
    spectrum.kmod[i] = 2*PI/L + 1.0*i/(N_kmod-1) * (1.41*100*2-2)*PI/L;
  }
  ...
  for (int i=0; i<N_theta-1; ++i){
    sum = sum + dtheta*0.5*(Dtheta[i+1]-Dtheta[i]);
  }
  for (int i=0; i<N_theta; ++i){Dtheta[i] = Dtheta[i]/sum;}
  ...
  for (int ix=0; ix<N_mode; ++ix){
    for (int iy=0; iy<(N_mode+1); ++iy){
      ...
      spectrum.F_kxky[ix*N_mode+iy] = 
        interp_lin(spectrum.kmod, theta, N_kmod, N_theta, rho, phi, 
            F_kmodtheta);
    }
  }
...
double wave(double x, double y, int N_grid, T_Spectrum spec, int N_mode, 
            double *phase){
  double eta = 0;
  double ampl = 0, a = 0;
  int index = 0;
  ...
  for (int i=0; i<N_mode; i++) {
    for (int j=0; j<N_mode+1; j++) {
      ...
      a = (spec.kx[i]*x + spec.ky[j]*y + phase[index]);
      eta += ampl*cos(a);
    }
  }
  return eta;
