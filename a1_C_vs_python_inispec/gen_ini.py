...
# saving also the F(k) for comparison
# Note: this code is the same as whats inside spectrum_gen_linear()
N_kmod = 64;
kmod = np.linspace(2*np.pi/L,1.41*100*2*np.pi/L,N_kmod)
F_k = spectrum_PM (P, kp, kmod)
...
if True:
    ...
    eta_save = np.zeros(N_grid**2)
    for i in range(N_grid):
        for j in range(N_grid):
            index = i*N_grid + j
            eta_save[index] = eta_tile[i,j] 
...
path = "./" # path of choice
file = "F_kxky_py" #_P%g_200m" %(P)
print('Written to', path+file)
fF = open(path + file, 'bw')
F_output = F_kxky_tile.astype('float64'); F_output.tofile(fF)
# output kx
file = "kx_py" #_200m"
print('Written to', path+file)
fx = open(path + file, 'bw')
F_output = kx.astype('float64'); F_output.tofile(fx)
# output ky
file = "ky_py" #_200m"
print('Written to', path+file)
fy = open(path + file, 'bw')
F_output = ky.astype('float64'); F_output.tofile(fy)
# output F(k)
file = "F_k_py" 
print('Written to', path+file)
fF = open(path + file, 'bw')
F_output = F_k.astype('float64'); F_output.tofile(fF)
# output k
file = "kmod_py" 
print('Written to', path+file)
fF = open(path + file, 'bw')
F_output = kmod.astype('float64'); F_output.tofile(fF)
# output eta
file = "eta_py"
print('Written to', path+file)
fF = open(path + file, 'bw')
F_output = eta_save.astype('float64'); F_output.tofile(fF)
